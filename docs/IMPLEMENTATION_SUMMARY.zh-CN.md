...
5. 增加了宿主机自动化代理：
   - 默认 `read-only`
   - 可升级到 `browser-write`
6. all-in-one 默认自动批准首个待配对设备，减少首次打开 Dashboard 时的配对阻塞
...
首次打开 Dashboard 时，用户最容易卡住的点就是：

- token 已经有了
- 但设备还没有被信任
- UI 停在 `pairing required`

为了把“一打开就能聊”做成默认体验，all-in-one 现在会在启动后短时间内轮询待配对设备列表：

- 如果已经有已配对设备，不做任何事
- 如果还没有已配对设备，但出现了待配对设备，就自动批准最新的一个
- 批准成功后立即停止，不会无限放开后续设备
